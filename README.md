...
- **framer-motion** - Animation library for 3D tilt cards (web only)
...
A drop-in 3D tilt card component with spring animations and choreographed fade-in effects (web only).

**Features:**
- ✨ 3D tilt effect on mouse hover with smooth spring physics
- ⚡ Spring-based hover animations with scale and shadow
- 🎬 Choreographed fade-in with staggered entrance for multiple cards
- 🎨 Works with Tailwind CSS or custom styles

**Quick Start:**

```tsx
import CraftCard from './src/components/CraftCard';
import './src/components/CraftCard.css';
...
For complete documentation, examples, and API reference, see [`src/components/README.md`](src/components/README.md).

**Note:** CraftCard uses `framer-motion` which is web-only. Use it with `expo start --web`. For native platforms, consider `moti` or `react-native-reanimated`.
