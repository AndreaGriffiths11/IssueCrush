...
describe('github api (AAA)', () => {
  ...
  it('fetchIssues filters out pull requests and returns issues (Arrange/Act/Assert)', async () => {
    // Arrange
    const mockIssues = [
      { id: 1, number: 1, title: 'I', pull_request: undefined },
    ...
    // Act
    const result = await fetchIssues('token');
  ...
  it('fetchIssues throws on 404 and 401 (Arrange/Act/Assert)', async () => {
    ...
    await expect(fetchIssues('t')).rejects.toThrow('Unauthorized');
  });
  ...
  it('updateIssueState sends PATCH and returns updated issue (Arrange/Act/Assert)', async () => {
    ...
    // Act
    const res = await updateIssueState('token', { number: 10, repository_url: 'https://api.github.com/repos/o/r' }, 'closed');
